...
The Krystal Programming Model is a developer-friendly way to write high-performant, complex business logic which pulls
in data in a scatter-gather fashion from multiple data sources and computes some output in a synchronous fashion. It is
optimized for writing business logic which can easily be reused across different applications, and allows for developers
to contribute to systems in a self-serve manner by hiding reactive programming and other runtime constructs.

Design Goals
============

The krystal programming model is design to prioritize the following key design goals:
...
2. Minimize lateral and upward impact of code changes: If all the business logic executing to fulfill a request is seen
...
3. Zero glue code: If there is business logic A and business logic B, all the relevant business logic should be
   contained within the artefact which represents each piece of business logic.
4. Self-serve programming model: The Krystal programming model is designed to allow for multiple developers from
   different disconnected teams to code, test and deploy business logic in which they are the domain expert into a
   common runtime environment at the same time minimizing chances of introducing regressions in functionality and
   performance of the applications in which the code is being deployed.
5. Cross-feature and Cross-application re-usability: The same piece of business logic can be reused for different
   requirements within an application and also across different application runtimes.
6. Out-of the box Non-intrusive batching of service-call inputs
7. Optimal end-to-end execution: This programming model is designed to be adopted for executing complex business logic
   in systems that power features and user experiences which directly are accessed by end users of Flipkart, thus
...
8. Avoid unnecessary bottlenecks and latency long poles: Generally, … [\[a\]](#cmnt1)
...
10. Observability - application owners get this Out of the Box:
    * Circuit Breaking
...
12. Programming language agnostic spec definitions: Although java is ubiquitous in Flipkart User-path services, keeping
...
theoretically be used within any runtime which supports the Krystal programming model (
Example: [Krystex](https://www.google.com/url?q=https://docs.google.com/document/d/1GF1A9sjx8LrXTU2J1j7TqfA5PFn1kPkNm_yUK1zZvNQ/edit%23heading%3Dh.9m165nnv4bfj&sa=D&source=editors&ust=1660198141937125&usg=AOvVaw0jATqkfBosGIxD2-gGRHGL))
