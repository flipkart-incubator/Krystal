 ...
 * custom annotations present in the java code written using the vajram-java-sdk. This annotation
 * has been created specifically to support scenarios where tagging needs to be done dynamically
 * (via a UI for example). Application developers and framework component developers are expected to
 * use custom annotations to annotate elements in code (This is the reason this annotaiton has
 * no @{@link Target}s specified).
 *
