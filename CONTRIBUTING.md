...
- Testing in krystal is done in two layers
    ...
    - End2End unit tests
- Module specific unit tests are present in the src/test directory of each framework module itself
  and are designed to give immediate feedback for locally testable features. But not all features -
...
- End2End unit tests are present in sample projects.
    ...
    - These sample projects also act as documentation for clients educating how to use krystal and
...
1. After making changes to the framework,
   ...
   projects
2. Run `./gradlew test --rerun-tasks` to run all tests.
3. Run `./gradlew build` to run all tests and all code analysis checks.