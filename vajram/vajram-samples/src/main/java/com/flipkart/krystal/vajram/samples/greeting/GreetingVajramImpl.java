...
public final class GreetingVajramImpl extends GreetingVajram {
  ...
  @Override
  public String executeNonBlocking(ExecutionContext executionContext) {
    GreetingVajramRequest _request = executionContext.getValue("_request");
  ...
  @Override
  public Object resolveInputOfDependency(
    ...
    switch (dependency) {
      case "user_info" -> {
        if (Set.of("user_id").equals(resolvableInputs)) {
          String user_id = super.userIdForUserService(executionContext.getValue("user_id"));
          return UserServiceVajramRequest.builder().userId(user_id);
        }
      }
    }
