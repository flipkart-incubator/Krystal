...
import com.flipkart.krystal.vajram.samples.sql.traits.CreateUserTrait_sql_Req;
  ...
  void setUp() {
    ...
    VajramKryonGraphBuilder graphBuilder =
        VajramKryonGraph.builder()
            .loadFromPackage(CreateUser.class.getPackageName())
    ...
    try (KrystexVajramExecutor vajramExecutor = graph.createExecutor(config)) {
      future =
          vajramExecutor.execute(
              CreateUser_ReqImmutPojo._builder()
                  .name("Test User")
