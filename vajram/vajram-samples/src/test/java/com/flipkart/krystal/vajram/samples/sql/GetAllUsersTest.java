...
import com.flipkart.krystal.vajram.samples.sql.traits.GetAllUsersTrait_sql_Req;
...
class GetAllUsersTest {
  ...
  private static final Logger logger = Logger.getLogger(GetAllUsersTest.class.getName());
  ...
  void setUp() {
    ...
    traitBinder.bindTrait(GetAllUsersTrait_Req.class).to(GetAllUsersTrait_sql_Req.class);
    // Load vajrams from the generated package and SQLRead package
    VajramKryonGraphBuilder graphBuilder =
        VajramKryonGraph.builder()
            .loadFromPackage(GetAllUsers.class.getPackageName())
  ...
  void getAllUsers_realDatabase_success() {
    ...
    try (KrystexVajramExecutor vajramExecutor = graph.createExecutor(config)) {
      future =
          vajramExecutor.execute(
              GetAllUsers_ReqImmutPojo._builder()._build(),
