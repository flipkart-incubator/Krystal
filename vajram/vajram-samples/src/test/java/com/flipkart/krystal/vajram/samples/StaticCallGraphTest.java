...
class StaticCallGraphTest {
  ...
  @Test
  void testStaticCallGraphContainsGivenStartVajram() {
    ...
    assertThat(htmlContent).as("HTML should contain Subtract as a dependency").contains("Subtract");

    assertThat(htmlContent).as("HTML should contain Multiply as a dependency").contains("Multiply");
  ...
  @Test
  void graphWithTraits_expandsDispatchTarget() {
    VajramGraph graph =
        VajramGraph.builder()
            .loadClasses(
  ...
  @Test
  void testEmptyGraphShowsNoVajramsMessage() {
    VajramGraph emptyGraph = VajramGraph.builder().build();
    KrystexGraphBuilder kGraph = KrystexGraph.builder().vajramGraph(emptyGraph);
