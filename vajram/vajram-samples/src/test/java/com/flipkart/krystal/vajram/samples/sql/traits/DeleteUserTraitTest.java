...
public class DeleteUserTraitTest {
  private static final Logger logger = Logger.getLogger(DeleteUserTrait.class.getName());
  ...
  void setUp() {
    ...
    traitBinder.bindTrait(DeleteUserTrait_Req.class).to(DeleteUserTrait_sql_Req.class);
    // Load vajrams from the generated package and SQLWrite package
    VajramKryonGraphBuilder graphBuilder =
        VajramKryonGraph.builder()
            .loadFromPackage(DeleteUserTrait.class.getPackageName())
    ...
    graph.registerTraitDispatchPolicies(
        new StaticDispatchPolicyImpl(
            graph, graph.getVajramIdByVajramDefType(DeleteUserTrait.class), traitBinder));
  }

  @Test
  @Disabled("Requires MySQL server - enable manually when database is available")
  void deleteUserSqlTrait_realDatabase_success() {
    // Configure connection to MySQL database
    logger.info("Initializing deleteUserSqlTrait test with real database");
    // Create the VajramKryonGraph
    KrystexVajramExecutorConfig config =
        KrystexVajramExecutorConfig.builder()
            .kryonExecutorConfigBuilder(
                KryonExecutorConfig.builder()
                    .executorId("deleteUserTest")
    ...
    try (KrystexVajramExecutor vajramExecutor = graph.createExecutor(config)) {
      future =
          vajramExecutor.execute(
              DeleteUserTrait_sql_ReqImmutPojo._builder().parameters(List.of(10))._build(),
              KryonExecutionConfig.builder().executionId("test-execution-delete-db").build());
    }

    // Assert results
    assertThat(future).succeedsWithin(TEST_TIMEOUT);
    Long deletedRows = future.join();

    assertThat(deletedRows).isNotNull();
  }
}
