...
public class VajramCodeGenUtility {
  ...
  private DefaultFacetModel toGivenFacetModel(
    ...
    CodeGenType dataType =
        facetField
            .asType()
            .accept(new DeclaredTypeVisitor(codegenUtil, facetField, DISALLOWED_FACET_TYPES), null);
  ...
  public static TypeName toTypeName(Type typeArg) {
    if (typeArg instanceof ParameterizedType parameterizedType) {
      ...
      return ParameterizedTypeName.get(
          ...
          stream(typeArgs).map(VajramCodeGenUtility::toTypeName).toArray(TypeName[]::new));
    } else {
