...
final class Proto3SchemaGen implements CodeGenerator {
  ...
  private void validate() {
    ...
    // Skip serde validation for enums - they don't have model methods
  ...
  static void validateModelType(TypeElement modelRootType, CodeGenUtility util) {
    if (ENUM.equals(modelRootType.getKind())) {
      // Enum models are valid for protobuf schema generation
  ...
  private void generateProtobufSchema() {
    ...
    try {
      ...
      // Generate proto file content - dispatch based on type
      String protoContent;
      if (util.isEnumModel(modelRootType)) {
  ...
  /**
   * Generates a proto3 enum definition for an EnumModel type. The proto index for enum values
   * matches the declaration order (ordinal) unless overridden by {@code @SerialId}.
   */
  private String generateEnumProtoFileContent(TypeElement enumType, String packageName) {
    ...
    String protoEnumName = enumName + MODELS_PROTO_MSG_SUFFIX;
    ...
    // Check if any constant has @SerialId
    ...
    for (int i = 0; i < enumConstants.size(); i++) {
      ...
      if (anyHasSerialId) {
        ...
        if (serialId != null) {
          ...
          // If some constants have @SerialId but this one doesn't, use ordinal
          protoIndex = i;
        }
      } else {
        // No @SerialId annotations: use ordinal (UNKNOWN = 0, next = 1, etc.)
        protoIndex = i;
      }
