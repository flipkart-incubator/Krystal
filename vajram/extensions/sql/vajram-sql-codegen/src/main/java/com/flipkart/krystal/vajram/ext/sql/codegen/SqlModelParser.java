...
public final class SqlModelParser {
  ...
  private static final String ORDER_BY_ANNO = ORDER_BY.class.getName();
  private static final String ORDER_BYS_ANNO = ORDER_BY.class.getName() + ".ORDER_BYs";
  private static final String LIMIT_ANNO = "com.flipkart.krystal.vajram.ext.sql.statement.LIMIT";

  private final ProcessingEnvironment env;

  public SqlModelParser(ProcessingEnvironment env) {
    this.env = env;
  }
  ...
  public TraitResultType parseTraitResultType(TypeElement traitElement) {
    for (TypeMirror iface : traitElement.getInterfaces()) {
      ...
      if (!te.getQualifiedName().contentEquals(TRAIT_DEF_CLASS)) continue;
      if (dt.getTypeArguments().isEmpty()) continue;
      TypeMirror arg = dt.getTypeArguments().get(0);
      if (!(arg instanceof DeclaredType argDt)) continue;
      if (!(argDt.asElement() instanceof TypeElement argElem)) continue;
      if (argElem.getQualifiedName().contentEquals("java.util.List")) {
        if (!argDt.getTypeArguments().isEmpty()) {
          ...
          TypeElement innerElem = (TypeElement) env.getTypeUtils().asElement(inner);
          if (innerElem != null) return new TraitResultType(innerElem, true);
        }
      } else {
  ...
  public List<WhereInput> collectWhereInputs(List<ExecutableElement> inputMethods) {
    ...
    for (ExecutableElement method : inputMethods) {
      ...
      if (!(rt instanceof DeclaredType dt)) continue;
      if (!(dt.asElement() instanceof TypeElement typeElem)) continue;
      if (!hasAnnotation(typeElem, WHERE_ANNO)) continue;
  ...
  private JoinRelation parseJoinRelation(
    ...
    if (childTableElem == null) return null;
    ...
    if (childFkCol == null || parentPkCol == null) return null;
    ...
    for (ExecutableElement childMethod :
      ...
      if (grandChildProjElem != null) {
        ...
        if (nested != null) nestedJoins.add(nested);
      }
    }
  ...
  public List<ScalarColumn> parseScalarColumns(TypeElement projectionElement) {
    ...
    for (ExecutableElement m : ElementFilter.methodsIn(projectionElement.getEnclosedElements())) {
      ...
      if (getListElementProjection(rt) != null) continue; // skip List<@Projection> join methods
  ...
  public String resolveColumnName(ExecutableElement method) {
    for (AnnotationMirror mirror : method.getAnnotationMirrors()) {
      if (!mirror.getAnnotationType().toString().equals(COLUMN_ANNO)) continue;
  ...
  public String getTableName(TypeElement tableElement) {
    for (AnnotationMirror mirror : tableElement.getAnnotationMirrors()) {
      if (!mirror.getAnnotationType().toString().equals(TABLE_ANNO)) continue;
  ...
  public TypeElement getAnnotationClassValue(TypeElement element, String annoFqn, String attrName) {
    for (AnnotationMirror mirror : element.getAnnotationMirrors()) {
      if (!mirror.getAnnotationType().toString().equals(annoFqn)) continue;
      for (Map.Entry<? extends ExecutableElement, ? extends AnnotationValue> e :
          mirror.getElementValues().entrySet()) {
        if (!e.getKey().getSimpleName().contentEquals(attrName)) continue;
        TypeMirror tm = (TypeMirror) e.getValue().getValue();
        return (TypeElement) env.getTypeUtils().asElement(tm);
      }
    }
    return null;
  }
  ...
  public boolean hasAnnotation(TypeElement element, String annoFqn) {
    for (AnnotationMirror mirror : element.getAnnotationMirrors()) {
      if (mirror.getAnnotationType().toString().equals(annoFqn)) return true;
    }
    return false;
  }
  ...
  public boolean isOptionalType(TypeMirror type) {
    ...
    if (!(dt.asElement() instanceof TypeElement te)) return false;
   ...
   */
  public TypeElement getListElementProjection(TypeMirror returnType) {
    if (!(returnType instanceof DeclaredType dt)) return null;
    if (!(dt.asElement() instanceof TypeElement te)) return null;
    if (!te.getQualifiedName().contentEquals("java.util.List")) return null;
    if (dt.getTypeArguments().isEmpty()) return null;
    TypeMirror inner = dt.getTypeArguments().get(0);
    TypeElement innerElem = (TypeElement) env.getTypeUtils().asElement(inner);
    if (innerElem != null && hasAnnotation(innerElem, PROJECTION_ANNO)) return innerElem;
    return null;
  }
  ...
  public String findFkColumnInChildForParent(TypeElement childTable, TypeElement parentTable) {
    for (ExecutableElement method : ElementFilter.methodsIn(childTable.getEnclosedElements())) {
      if (!hasAnnotationOnElement(method, FOREIGN_KEY_ANNO)) continue;
      if (!(method.getReturnType() instanceof DeclaredType dt)) continue;
      if (!(dt.asElement() instanceof TypeElement refElem)) continue;
   ...
   * </ul>
   *
   * <p>Errors are reported via the processing environment's {@link
   * javax.tools.Diagnostic.Kind#ERROR ERROR} messager so that the compiler surfaces them as build
   * failures.
   */
  public void validateTableAndWhereElements(RoundEnvironment roundEnv) {
    TypeElement tableModelType = env.getElementUtils().getTypeElement(TABLE_MODEL_FQN);
    TypeElement whereClauseType = env.getElementUtils().getTypeElement(WHERE_CLAUSE_FQN);

    for (Element element : roundEnv.getElementsAnnotatedWith(Table.class)) {
      if (!(element instanceof TypeElement te)) continue;
      ...
      if (tableModelType != null
          && !env.getTypeUtils().isAssignable(te.asType(), tableModelType.asType())) {
        env.getMessager()
            .printMessage(
                Kind.ERROR,
                "[vajram-sql] @Table interface '"
                    + te.getQualifiedName()
                    + "' must extend TableModel.",
                te);
      }
    }
    ...
    for (Element element : roundEnv.getElementsAnnotatedWith(WHERE.class)) {
      ...
      if (whereClauseType != null
          && !env.getTypeUtils().isAssignable(te.asType(), whereClauseType.asType())) {
        env.getMessager()
            .printMessage(
                Kind.ERROR,
                "[vajram-sql] @WHERE interface '"
  ...
  public boolean whereClauseCoversSingleRow(
    ...
    if (pkCol != null && whereFields.contains(pkCol)) return true;

    for (ExecutableElement method : ElementFilter.methodsIn(tableElement.getEnclosedElements())) {
      if (hasAnnotationOnElement(method, UNIQUE_KEY_ANNO)) {
        if (whereFields.contains(method.getSimpleName().toString())) return true;
      }
    }

    for (AnnotationMirror mirror : tableElement.getAnnotationMirrors()) {
      if (!mirror.getAnnotationType().toString().equals(UNIQUE_KEY_ANNO)) continue;
      for (Map.Entry<? extends ExecutableElement, ? extends AnnotationValue> e :
          mirror.getElementValues().entrySet()) {
        if (!e.getKey().getSimpleName().contentEquals("columns")) continue;
        ...
        if (!keyColumns.isEmpty() && whereFields.containsAll(keyColumns)) return true;
      }
    }
  ...
  public List<String> findInvalidProjectionColumns(
    ...
    for (ExecutableElement m : ElementFilter.methodsIn(projectionElement.getEnclosedElements())) {
      if (getListElementProjection(m.getReturnType()) != null) continue;
   ...
   *
   * <p>For example, {@code extends TraitDef<@ORDER_BY(column="name", order=ASC) UserInfo>} will
   * return a single {@link OrderByClause}. Multiple {@code @ORDER_BY} annotations are supported via
  ...
  public List<OrderByClause> parseOrderBysFromTraitTypeArg(TypeElement traitElement) {
    ...
    if (typeArg == null) return List.of();
    ...
    for (AnnotationMirror mirror : typeArg.getAnnotationMirrors()) {
      ...
      if (type.equals(ORDER_BY_ANNO)) {
        ...
        for (AnnotationValue containerVal : mirror.getElementValues().values()) {
          if (!(containerVal.getValue() instanceof List<?> list)) continue;
  ...
  public int parseLimitFromTraitTypeArg(TypeElement traitElement) {
    TypeMirror typeArg = findTraitDefTypeArg(traitElement);
    if (typeArg == null) return -1;
    for (AnnotationMirror mirror : typeArg.getAnnotationMirrors()) {
      if (!mirror.getAnnotationType().toString().equals(LIMIT_ANNO)) continue;
  ...
  public boolean hasLimitAnnotationOnTraitTypeArg(TypeElement traitElement) {
    ...
    for (AnnotationMirror mirror : typeArg.getAnnotationMirrors()) {
      if (mirror.getAnnotationType().toString().equals(LIMIT_ANNO)) return true;
    }
    return false;
  }
