...
an @[SQL](src/main/java/com/flipkart/krystal/vajram/ext/sql/lang/SQL.java) trait which defines
...
Similarly, [Selection](src/main/java/com/flipkart/krystal/vajram/ext/sql/model/Selection.java)
...
| `@UniqueKey(name, columns = {...})` | `@Table(uniqueKeys = ...)` attribute | Composite unique key across multiple columns                                                    |
...
@Table(
    name = "user_emails",
    uniqueKeys = @UniqueKey(name = "uk_user_email_type", columns = {"userId", "emailType"}))
...
`@Selection(from = TableClass.class)`](src/main/java/com/flipkart/krystal/vajram/ext/sql/model/Selection.java).
...
`@Column("columnName")`](src/main/java/com/flipkart/krystal/vajram/ext/sql/model/Column.java).
...
- A `List<AnotherSelection>` method in a parent selection signals a LEFT JOIN. The child
  selection must also be annotated with `@Selection(from = ChildTable.class)`.
...
- A **list-result** trait (`TraitDef<List<T>>`) **must** declare `@LIMIT` on the type argument.
  ...
  `[SqlTraitVajramGen] ... returns a List but has no @LIMIT` (emitted by `SqlSelectVajramGen`, which retains this string as a log prefix). Use `@LIMIT(LIMIT.NO_LIMIT)` to
...
| `@IsLessThan`           | `<`          | Same as `@IsGreaterThan`                                                                               |
...
public interface RecentOrdersPredicate extends ColumnPredicate {
...
1. Define individual predicates as `ColumnPredicate` interfaces:
...
- Each method in an `SqlOrPredicate` must return a `ColumnPredicate` type (with `@WHERE`).
...
- **`POSTGRESQL_18` / `SQL_LITE_3_35`** — The generated SQL includes a `RETURNING <column>` clause.
  The `mapResult` method iterates over the returned rows to build the result.
- **`SQL_2023`** — No `RETURNING` clause is generated (the standard doesn't define it).
...
Auto-assigned columns (e.g. auto-increment IDs) are declared with
`@DefaultValueStrategy(AUTO_ASSIGN_ID)` on the table column, not with a dedicated annotation.
...
Create a `@WHERE` interface for each group of WHERE predicates. Use `ColumnPredicate` with
...
| Fetch multiple rows (bounded)                 | Scalar methods only               | `TraitDef<@LIMIT(N) List<T>>`             | **Required** — `SqlSelectVajramGen` emits a compile error (logged with the `[SqlTraitVajramGen]` prefix) if `TraitDef<List<T>>` has no `@LIMIT`; use `@LIMIT(N)` to cap at N rows                                                                          |
