...
The `_ImmutPojo` classes are generated by Krystal's `ModelGenProcessor`. Each projection model must declare `@SupportedModelProtocol(PlainJavaObject.class)` to trigger their generation. This applies to all projections — including nested ones used in JOINs (e.g. `OrderInfo` above).
