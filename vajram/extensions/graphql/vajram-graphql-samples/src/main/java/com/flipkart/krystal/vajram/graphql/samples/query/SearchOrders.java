 ...
 * DateRangeInput}) and uses it to filter and return a list of OrderIds. The aggregator will then
 * use these IDs to fetch the full Order entities.
 */
@GraphQLFetcher
@Vajram
public abstract class SearchOrders extends ComputeVajramDef<List<OrderId>> {
   ...
   * @return List of matching OrderIds
   */
  @Output
  static List<OrderId> searchOrders(@Nullable OrderFilterInput filter) {
    // If no filter provided, return sample results
    if (filter == null) {
      return List.of(new OrderId("order_1"), new OrderId("order_2"), new OrderId("order_3"));
    }
    ...
    if (status != null && !status.isBlank()) {
      return List.of(
          new OrderId("order_" + status.toLowerCase() + "_1"),
          new OrderId("order_" + status.toLowerCase() + "_2"));
    }

    // Example: Filter by minimum amount
    Float minAmount = filter.minAmount();
    if (minAmount != null) {
      return List.of(new OrderId("order_amount_gt_" + minAmount));
    }
    ...
    if (dateRange != null) {
      ...
      if (from != null && to != null) {
        return List.of(new OrderId("order_" + from + "_to_" + to));
      } else if (from != null) {
        return List.of(new OrderId("order_after_" + from));
      } else if (to != null) {
        return List.of(new OrderId("order_before_" + to));
      }
    }

    // Default: return sample results
    return List.of(new OrderId("order_default"));
  }
}
