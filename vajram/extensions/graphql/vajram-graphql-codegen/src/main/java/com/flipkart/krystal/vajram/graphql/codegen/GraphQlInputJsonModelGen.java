...
final class GraphQlInputJsonModelGen implements CodeGenerator {
  ...
  GraphQlInputJsonModelGen(ModelsCodeGenContext codeGenContext) {
  ...
  public void generate() {
    ...
    ClassName immutableJsonName =
        ClassName.get(
            packageName, immutableModelName + GraphQlInputJson.INSTANCE.modelClassesSuffix());
  ...
  private boolean isApplicable() {
    return CodegenPhase.MODELS.equals(codeGenContext.codegenPhase())
        && isGraphQlInputJsonSerdeSupported();
  }

  /**
   * Checks if the model root supports GraphQlInputJson serialization.
   *
   * @return true if GraphQlInputJson is supported, false otherwise
   */
  private boolean isGraphQlInputJsonSerdeSupported() {
    TypeElement modelRootType = codeGenContext.modelRootType();
    ...
    // Check if GraphQlInputJson is mentioned in the annotation value
    return util.getTypesFromAnnotationMember(supportedModelProtocols::value).stream()
        ...
        .anyMatch(GraphQlInputJson.class.getCanonicalName()::equals);
  }
}
