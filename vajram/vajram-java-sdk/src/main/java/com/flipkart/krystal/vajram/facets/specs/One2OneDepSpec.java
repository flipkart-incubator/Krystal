...
public abstract sealed class One2OneDepSpec<T, CV extends Request, DV extends Request<T>>
  ...
  @Override
  public One2OneDepResponse<DV, T> getFacetValue(FacetValues facetValues) {
    return getFromFacets.apply(facetValues);
  }

  @Override
  @SuppressWarnings("unchecked")
  public final void setFacetValue(FacetValuesBuilder facets, DepResponse<DV, T> value) {
    if (value instanceof One2OneDepResponse<?, ?> one2OneDepResponse) {
      setFacetValue(facets, (One2OneDepResponse<DV, T>) one2OneDepResponse);
    } else {
  ...
  public final void setFacetValue(FacetValuesBuilder facets, One2OneDepResponse<DV, T> value) {
